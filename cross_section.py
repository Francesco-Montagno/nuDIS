...
parser.add_argument('-f', '--folder', type=str, default=None,
                    help='Output folder path (overrides auto-generated timestamped folder)')
parser.add_argument('--process', type=str, default=None,
                    help='Process to compute (overrides process from card)')
...
# --- Process and Beam ---
process_name   = card.get("process",       args.process)  # str: eg "cbar_p"
E_muon         = card.get("E_muon")                       # float: 1500.0

# --- PDF Settings ---
replica_id     = card.get("replica",       args.replica)  # int: 0
...
# --- Binning: X ---
num_bins_x     = card.get("num_bins_x",     args.num_bins_x)  # int: 1
x_min_bin      = card.get("x_min_bin",     args.x_min_bin)  # float: 0.001
x_max_bin      = card.get("x_max_bin",     args.x_max_bin)  # float: 1.0

# --- Binning: Q2 ---
num_bins_Q2    = card.get("num_bins_Q2",    args.num_bins_Q2)  # int: 1
Q2_min_bin     = card.get("Q2_min_bin",    args.Q2_min_bin)   # float: 1.0
Q2_max_bin     = card.get("Q2_max_bin",    args.Q2_max_bin)   # float: 3000.0
...
if __name__ == '__main__':
    ...
    print(f"[resolved] num_bins_x={num_bins_x}  x=[{x_min_bin}, {x_max_bin}]")
    print(f"[resolved] num_bins_Q2={num_bins_Q2}  Q2=[{Q2_min_bin}, {Q2_max_bin}]")
    #----------------------------------- Binning ------------------------------------#
    ...
    def compute_bin(m):
        ...
        sigma = integral(crossSection[process], nitn=n_iter, neval=n_eval, adapt=True, adapt_to_errors=False)
    ...
    folder = Path(args.folder) if args.folder else Path("data", "output") / datetime.now().strftime("%Y%m%d_%H%M%S")
    ...
    effective = {
        "process":    process_name,
        "replica":    replica_id,
        "pdf_set":    pdf_set_name,
        "num_bins_x": num_bins_x,
        "x_min_bin":  x_min_bin,
        "x_max_bin":  x_max_bin,
        "num_bins_Q2": num_bins_Q2,
        "Q2_min_bin": Q2_min_bin,
        "Q2_max_bin": Q2_max_bin,
    }
