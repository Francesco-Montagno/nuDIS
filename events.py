...
parser.add_argument('-f', '--folder', type=str, default=None,
                    help='Folder in which to save the output (overrides process name)')
...
parser.add_argument('--E_muon', type=float, default=None,
...
parser.add_argument('--Q2_min_theory', type=float, default=None,
                    help='Minimum Q2 for theoretical cuts (overrides Q2_min_theory from card)')
parser.add_argument('--W2_min_theory', type=float, default=None,
                    help='Minimum W2 for theoretical cuts (overrides W2_min_theory from card)')
parser.add_argument('--vegas_iter', type=int, default=None,
                    help='Number of VEGAS iterations (overrides vegas_iter from card)')
parser.add_argument('--vegas_eval', type=int, default=None,
...
parser.add_argument('--pdf_set', type=str, default=None,
                    help='LHAPDF PDF set name (overrides pdf_set from card)')
...
parser.add_argument('--E_min_bin', type=float, default=None,
                    help='Minimum energy for binning (overrides E_min_bin from card)')
parser.add_argument('--E_max_bin', type=float, default=None,
...
# CLI args take precedence over card values via override parameter
process_name  = card.get("process",       args.process)
E_muon        = card.get("E_muon",        args.E_muon)
replica_id    = card.get("replica",       args.replica)
Q2_min_theory = card.get("Q2_min_theory", args.Q2_min_theory)
W2_min_theory = card.get("W2_min_theory", args.W2_min_theory)
n_iter        = card.get("vegas_iter",    args.vegas_iter)
n_eval        = card.get("vegas_eval",    args.vegas_eval)
ncores        = card.get("ncores",        args.ncores)
n_events      = card.get("n_events",      args.n_events)
pdf_set_name  = card.get("pdf_set",       args.pdf_set)
x_min_bin     = card.get("x_min_bin",     args.x_min_bin)
x_max_bin     = card.get("x_max_bin",     args.x_max_bin)
Q2_min_bin    = card.get("Q2_min_bin",    args.Q2_min_bin)
Q2_max_bin    = card.get("Q2_max_bin",    args.Q2_max_bin)
E_min_bin     = card.get("E_min_bin",     args.E_min_bin)
E_max_bin     = card.get("E_max_bin",     args.E_max_bin)
...
import src.pdfs.pdf_set as _pdf_module
_pdf_module.configure(pdf_set_name)
...
if __name__ == '__main__':
    ...
    folder  = Path("data", "output") / (args.folder or process_name)
    folder.mkdir(parents=True, exist_ok=True)
    print(f"Created folder: {folder}")

    integral = vegas.Integrator([[x_min_bin, x_max_bin], [Q2_min_bin, Q2_max_bin], [E_min_bin, E_max_bin]])

    # Adaptive integration to build a good grid
    sigma = integral(crossSection[process], nitn=n_iter, neval=n_eval, adapt=True, adapt_to_errors=False, nproc=ncores)
    # Final integration with fixed grid
